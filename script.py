...
def vg_decrypt(ciphertext, key, abool=True):
    ...
    return vg(ciphertext, inverse, abool)

def vg(plaintext, key, abool=True):
    key = key.lower()
...
def test_vg(text, key, abool=True):
    ciphertext = vg(text, key, abool)
    plaintext  = vg_decrypt(ciphertext, key, abool)
    assert plaintext == text, "{!r} -> {!r} -> {!r} (a {}= 0)".format(
        text, ciphertext, plaintext, "" if abool else "!")

for text in ["rewind", "text with spaces", "pun.ctuation", "numb3rs"]:
    for key in ["iepw", "aceaq", "safe", "pwa"]:
        test_vg(text, key, True)
        test_vg(text, key, False)
...
def solve_vg(text, key_min_size=1, key_max_size=12, abool=True):
    ...
    for key_length in range(key_min_size, key_max_size + 1):
        ...
        for key_index in range(key_length):
            ...
            for key_char in string.ascii_lowercase:
                shifts.append(
                    (analyse_frequency(vg_decrypt(letters, key_char, abool)), key_char)
                )
            key[key_index] = min(shifts, key=lambda x: x[0])[1]
        best_keys.append("".join(key))
    best_keys.sort(key=lambda key: analyse_frequency(vg_decrypt(text, key, abool)))
    return best_keys[:2]
ENGLISH_WORDS = {
...
def map_with_dictionary(text):

    words = text.lower().split()
    meaningful_word_count = sum(1 for word in words if word in ENGLISH_WORDS)
    return meaningful_word_count >= len(words) * 0.4 

def main():
    ciphertext = input("Enter the ciphertext to decrypt: ").strip()

    for key in reversed(solve_vg(ciphertext)):
        decrypted_text = vg_decrypt(ciphertext, key)
        if ciphertext == vg(decrypted_text, key) and map_with_dictionary(decrypted_text):
            print("")
